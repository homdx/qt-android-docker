...
RUN mkdir -p /tmp/qt-installer \
       ...
       && rm -rf /tmp/qt-installer &&  apt install build-essential g++ -y && \
...
make -j4 && echo end build && date && echo build done && make install && cd /Qt/5.13.1/Src/qtbase/src/tools/androiddeployqt && make && make install &&  echo done1 && date && rm -rf /Qt && date && echo all done ok || echo error build
